...
#
# Prerequisites: lotus + yugabyte/curio stack running; cast + jq installed; sp.key present.
# Funds ./client.key and ./client2.key when those files exist (at least one required).
#
# Usage (from repo root):
#   ./scripts/fund-devnet-wallets.sh
#
# Overrides:
#   CLIENT_KEY / CLIENT2_KEY / SP_KEY  private key files
#                                      (default ./client.key, ./client2.key, ./sp.key)
...
#   USDFC_AMOUNT                 whole USDFC tokens per client wallet (default 1000)
...
CLIENT2_KEY="${CLIENT2_KEY:-$ROOT/client2.key}"
...
for f in "$SP_KEY" "$CONTRACTS_DIR/contract_addresses.json" "$CONTRACTS_DIR/deployer.private-key"; do
...
# label:path pairs for client wallets that exist on disk.
CLIENT_KEYS=()
[[ -f "$CLIENT_KEY" ]] && CLIENT_KEYS+=("client:$CLIENT_KEY")
[[ -f "$CLIENT2_KEY" ]] && CLIENT_KEYS+=("client2:$CLIENT2_KEY")
[[ ${#CLIENT_KEYS[@]} -gt 0 ]] || {
  echo "missing client key file(s): need at least one of $CLIENT_KEY or $CLIENT2_KEY" >&2
  exit 1
}
...
CLIENT_ADDRS=()
CLIENT_LABELS=()
for entry in "${CLIENT_KEYS[@]}"; do
  label="${entry%%:*}"
  path="${entry#*:}"
  addr="$(cast wallet address --private-key "0x$(hex_key "$path")")"
  CLIENT_LABELS+=("$label")
  CLIENT_ADDRS+=("$addr")
  echo "${label}=${addr} (from ${path})"
done
echo "sp=$SP"
echo "usdfc=$USDFC"
echo "rpc=$RPC"
...
echo "sending ${FIL_AMOUNT} FIL to client wallet(s) and sp..."
FIL_CIDS=()
for addr in "${CLIENT_ADDRS[@]}"; do
  cid="$(docker exec "$LOTUS_CONTAINER" lotus send "$addr" "$FIL_AMOUNT" | awk '/^bafy|^bafk/{print $1; exit}')"
  FIL_CIDS+=("$cid")
done
CID_S="$(docker exec "$LOTUS_CONTAINER" lotus send "$SP" "$FIL_AMOUNT" | awk '/^bafy|^bafk/{print $1; exit}')"
for cid in "${FIL_CIDS[@]}"; do
  wait_msg "$cid"
done
wait_msg "$CID_S"

for i in "${!CLIENT_ADDRS[@]}"; do
  echo "${CLIENT_LABELS[$i]} FIL: $(cast balance --ether --rpc-url "$RPC" "${CLIENT_ADDRS[$i]}")"
done
...
for i in "${!CLIENT_ADDRS[@]}"; do
  echo "transferring ${USDFC_AMOUNT} USDFC to ${CLIENT_LABELS[$i]}..."
  cast send "$USDFC" "transfer(address,uint256)" "${CLIENT_ADDRS[$i]}" "$USDFC_BASE_UNITS" \
    --rpc-url "$RPC" \
    --private-key "$DEPLOYER_KEY" \
    >/dev/null
  bal="$(cast call "$USDFC" "balanceOf(address)(uint256)" "${CLIENT_ADDRS[$i]}" --rpc-url "$RPC")"
  echo "${CLIENT_LABELS[$i]} USDFC (base units): $bal"
done
echo "done"
