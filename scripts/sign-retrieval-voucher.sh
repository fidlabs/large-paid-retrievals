#!/usr/bin/env bash
# Sign an EIP-712 PoRepPieceAccess RetrievalVoucher capability and print the
# base64url-nopad compact JSON token (typed data + signature). Clients mint a
# per-piece RetrievalProof and combine via sign-retrieval-credential.sh or
# retrieval-client --voucher binding.
#
# Usage:
...
#
# --deal-id is accepted as an alias for --scope (legacy).
...
# Compact capability token without EIP712Domain in types (matches pieceaccess).
