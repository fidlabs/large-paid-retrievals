...
func TestQuotePathRateLimitedPerIP(t *testing.T) {
	s := newTestStore(t)
	defer s.Close()
	h := newTestHandler(pp.Config{
		...
		QuotePayee0x: testQuotePayee0x,
		Store:        s,
		// Tiny refill so the burst is effectively the whole budget for the test window.
		QuoteRateLimitRPS:   0.001,
		QuoteRateLimitBurst: 2,
	})
	const cid = "bafkreidde4sfyosf2pm6u4vxb65wogjg464a6y6tcg75opo6q5wv34bley"

	quote := func(remoteAddr string) int {
		req := httptest.NewRequest(http.MethodGet, "/piece/"+cid+"?client=0x1111111111111111111111111111111111111111", nil)
	...
	// Burst of 2 anonymous quotes is allowed (402), the 3rd from the same IP is throttled.
	if code := quote("198.51.100.9:5000"); code != http.StatusPaymentRequired {
		t.Fatalf("quote 1 = %d want 402", code)
	}
	if code := quote("198.51.100.9:5001"); code != http.StatusPaymentRequired {
		t.Fatalf("quote 2 = %d want 402", code)
	}
	if code := quote("198.51.100.9:5002"); code != http.StatusTooManyRequests {
		t.Fatalf("quote 3 = %d want 429", code)
	}

	// A different client IP has an independent bucket.
	if code := quote("203.0.113.5:6000"); code != http.StatusPaymentRequired {
		t.Fatalf("other-IP quote = %d want 402", code)
	}

	// The authenticated/paid path is NOT gated by the quote limiter: an authed request
	// from the throttled IP reaches credential verification (402 malformed), never 429.
	authReq := httptest.NewRequest(http.MethodGet, "/piece/"+cid, nil)
	authReq.Host = "example.com"
	authReq.RemoteAddr = "198.51.100.9:5003"
	authReq.Header.Set("Authorization", "Payment not-a-valid-credential")
	authRec := httptest.NewRecorder()
	h.ServeHTTP(authRec, authReq)
	if authRec.Code == http.StatusTooManyRequests {
		t.Fatal("authenticated path must not be rate-limited by the quote limiter")
	}
	if authRec.Code != http.StatusPaymentRequired {
		t.Fatalf("authed malformed credential = %d want 402", authRec.Code)
	}
}

func TestQuotePathRateLimitDisabledByDefault(t *testing.T) {
	s := newTestStore(t)
	defer s.Close()
	// No QuoteRateLimit* set → limiter disabled → many anonymous quotes all pass.
	h := newTestHandler(pp.Config{FilecoinPay: &mockPaySettler{}, QuotePayee0x: testQuotePayee0x, Store: s})
	const cid = "bafkreidde4sfyosf2pm6u4vxb65wogjg464a6y6tcg75opo6q5wv34bley"
	for i := 0; i < 25; i++ {
		req := httptest.NewRequest(http.MethodGet, "/piece/"+cid+"?client=0x1111111111111111111111111111111111111111", nil)
		req.Host = "example.com"
		req.RemoteAddr = "198.51.100.9:7000"
		rec := httptest.NewRecorder()
		h.ServeHTTP(rec, req)
		if rec.Code != http.StatusPaymentRequired {
			t.Fatalf("quote %d = %d want 402 (limiter should be disabled)", i, rec.Code)
		}
	}
}
