...
## 6. Credential tokens & presentation

Each credential is a **standalone** EIP-712 object — `domain`, `types`, `primaryType`, `message`,
**and its own `signature` embedded inside** — serialized as base64url(JSON), no padding. The proof
and the voucher are **separate** tokens sent in **separate** headers; there is no enclosing token and
no `scheme` field.

**RetrievalProof** (exactly one):

```json
{
  "domain": { "name": "CurioRetrieval", "version": "1", "chainId": 314159, "verifyingContract": "0x<service>" },
...
**RetrievalVoucher** (zero or more; omitted for owner-direct access):
...
- `Authorization: RetrievalProof <token>` — exactly one; the recovered signer is the requester.
- `Authorization: RetrievalVoucher <token>` — **repeatable**; a client MAY present many vouchers
  (it need not know which scope a piece belongs to). The gate verifies them best-effort and uses
  whichever authorizes.
...
For a request on resource CID `R`, parse the one `RetrievalProof` token `P` and every
`RetrievalVoucher` token `V₀…Vₙ`:

1. **Proof (fatal).** `P` MUST be a complete EIP-712 object (`primaryType` = `RetrievalProof`,
   `signature` present); `P.resource == R` (byte-exact); `now ≤ P.deadline ≤ now + MAX_PROOF_TTL`;
   `P.domain` matches the gate’s pin. `requester = ecrecover(EIP712(P), P.signature)`. `P.scope` is
   advisory (the scope/deal is resolved from `R`). Any failure → reject.
2. **Vouchers (best-effort).** Each `Vᵢ` is verified independently (`primaryType` =
   `RetrievalVoucher`, domain pin, `now ≤ deadline`, valid signature →
   `owner = ecrecover(EIP712(Vᵢ), Vᵢ.signature)`). A voucher that fails any check is dropped (kept
   only as a diagnostic), never failing the request on its own.
3. Resolve the access-controlled scopes containing `R` (per service). If `R` is in any public scope
   → **serve** (public-wins). Else let `PRIV` be the controlling scopes, each with an `owner`.
4. For a scope `s ∈ PRIV` with owner `O`, **authorize** if either:
   - **owner-direct:** `requester == O`; or
   - **delegated:** some verified voucher `V` has `V.owner == O`, `V.grantee == requester`, and
     `V.scope == s`.
5. Else **deny** (403).

Response codes: missing/invalid proof → **403** (JSON diagnostic); present but unauthorized → **403**;
...
Each object embeds its own `signature` (§6) and is sent in its own header:
`Authorization: RetrievalProof <proof>` plus one `Authorization: RetrievalVoucher <voucher>`.
