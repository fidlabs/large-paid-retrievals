...
// startSIGUSR1StateDump logs deals and settlement pool rows when the process receives SIGUSR1.
...
func startSIGUSR1StateDump(store dealstore.DealStore, logger *slog.Logger) {
	...
	go func() {
		for range ch {
			logger.Info("SIGUSR1 received; dumping deals and settlement pool state")
